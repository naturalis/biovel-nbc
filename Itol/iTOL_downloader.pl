...
#obtained from http://itol.embl.de/help/iTOL_batch.zip
...
Getopt::Regex::GetOptions(\@ARGV,
						  ...
						  ['--?collapsedSmall=?',\$collapsedSmall, 1],
...
if (-e $configFile) {
	...
	$collapsedSmall = $cfg->read('collapsedSmall')  if ($cfg->read('collapsedSmall') );
...
if (defined $collapsedSmall) { $post_content{'collapsedSmall'} = $collapsedSmall; } 
